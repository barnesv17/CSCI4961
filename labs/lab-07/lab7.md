...
if __name__ == '__main__':
    ...
    for (source, target) in [('chaos', 'order'),
                             ('nodes', 'graph'), 
			     ('moron', 'smart'), ('flies','swims'), ('mango', 'peach'),
                             ('pound', 'marks')]:
...
def words_graph():
    """Return the words example graph from the Stanford GraphBase"""
    fh = gzip.open('words4_dat.txt.gz', 'r') # Change to words4_dat.txt.gz
    words = set()
    for line in fh.readlines():
        line = line.decode()
        if line.startswith('*'):
            continue
        w = str(line[0:4]) #Change from 0:5 to 0:4
        words.add(w)
    return generate_graph(words)
...
if __name__ == '__main__':
    G = words_graph()
    print("Loaded words4_dat.txt containing four-letter English words.") # Changed
    print("Two words are connected if they differ in one letter.")
    print("Graph has %d nodes with %d edges"
          % (nx.number_of_nodes(G), nx.number_of_edges(G)))
    print("%d connected components" % nx.number_connected_components(G))
